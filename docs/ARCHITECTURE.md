...
`implementation-plan` 的 `BRANCH_GATE` 会读取 `apps.json` 的 `primary`、`collaborators` 与 `repositories`，为每个可写服务切换或创建 `fun_<版本>_<需求编号>_<标题>_<Git 用户名>` 分支。创建时先按数字版本选择最新的 `origin/v*-release`，没有版本 release 分支时回退到 `origin/master`，最后以 `git push -u origin <branch>` 只发布功能分支；工作流不会创建或推送 `release` 别名。
